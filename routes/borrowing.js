      ...
      try {
        ...
        await connection.execute(
          `
                ...
                WHERE id = ?
            `,
          [isOverdue ? "returned" : "returned", fineAmount, borrowing_id],
        )
        ...
        // Create fine record ONLY if there's actually a fine
        if (fineAmount > 0) {
          await connection.execute(
            `
        ...
        if (fineAmount > 0) {
          message += `. Fine of $${fineAmount} applied for ${Math.ceil((returnDate.getTime() - dueDate.getTime()) / (1000 * 60 * 60 * 24))} days overdue.`
        } else {
        ...
        res.json({
          ...
          data: {
            fine_amount: fineAmount,
            is_overdue: isOverdue,
            days_overdue: isOverdue ? Math.ceil((returnDate.getTime() - dueDate.getTime()) / (1000 * 60 * 60 * 24)) : 0,
          },
        })
      } catch (error) {
        await connection.rollback()
